...
CSS="$ROOT/src/pages/AdminDashboard.login-luxury-v1.css"
BACKUP="$(mktemp -d /tmp/67-admin-login-luxury-v1.XXXXXX)"
CSS_EXISTED=0
...
if [ -f "$CSS" ]; then CSS_EXISTED=1; cp "$CSS" "$BACKUP/AdminDashboard.login-luxury-v1.css"; fi

restore() {
  cp "$BACKUP/AdminDashboard.jsx" "$JSX"
  if [ "$CSS_EXISTED" = "1" ]; then cp "$BACKUP/AdminDashboard.login-luxury-v1.css" "$CSS"; else rm -f "$CSS"; fi
}
trap restore ERR

curl -fsSL https://raw.githubusercontent.com/mohamedamouseo-a11y/67-patchs/main/67-admin-login-luxury-reference-v1.css -o "$CSS"
...
login_import = "import './AdminDashboard.login-luxury-v1.css';"
...
grep -q "AdminDashboard.login-luxury-v1.css" "$JSX"
...
echo "PATCH=67-ADMIN-LOGIN-LUXURY-REFERENCE-V1"
