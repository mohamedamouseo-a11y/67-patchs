...
PATCH_ASSET_COMMIT=eca748fb248017aff33f6b40645e298967e41d16
...
BACKUP_DIR="/tmp/67-admin-v17-backup-$$"
FAILED_STEP=init
MUTATED=0
...
rollback(){
  ...
  if [ "$MUTATED" = "1" ]; then
    ...
    npm run build >/tmp/67-admin-v17-rollback-build.log 2>&1 || true
  ...
  echo "ERROR=V17_CLEAN_CINEMATIC_ASSETS_FAILED_EXIT_${code}"
  exit "$code"
}
trap 'rollback $?' ERR
...
FAILED_STEP=download_clean_payload_parts
BASE_URL="https://raw.githubusercontent.com/mohamedamouseo-a11y/67-patchs/${PATCH_ASSET_COMMIT}/assets/v17"
for part in hero.b64.00 hero.b64.01 hero.b64.02 hero.b64.03 health.b64.00 health.b64.01; do
  curl -fsSL "${BASE_URL}/${part}" -o "/tmp/67-v17-${part}"
done
cat /tmp/67-v17-hero.b64.00 /tmp/67-v17-hero.b64.01 /tmp/67-v17-hero.b64.02 /tmp/67-v17-hero.b64.03 > /tmp/67-v17-hero.b64
cat /tmp/67-v17-health.b64.00 /tmp/67-v17-health.b64.01 > /tmp/67-v17-health.b64

FAILED_STEP=decode_and_verify_clean_assets
python3 - <<'PY'
from pathlib import Path
import base64, hashlib
specs = [
    ('HERO','/tmp/67-v17-hero.b64','/tmp/67-v17-hero.jpg',15668,11751,'9f0d19b07bb0af0494b5768c7d43a21cb4565f3c05b096c3b665fcecc5d4da65'),
    ('HEALTH','/tmp/67-v17-health.b64','/tmp/67-v17-health.jpg',5540,4154,'f2c167f8c9f9b54508dc00aad9f956321627361d70ed53a2a37866ad5b35c9db'),
]
for label,src,dst,expected_b64_len,expected_bytes,expected_sha in specs:
    raw=''.join(Path(src).read_text().split())
    if len(raw) != expected_b64_len:
        raise SystemExit(f'{label}_BASE64_LENGTH_MISMATCH:{len(raw)}:{expected_b64_len}')
    data=base64.b64decode(raw, validate=True)
    if len(data) != expected_bytes:
        raise SystemExit(f'{label}_JPEG_SIZE_MISMATCH:{len(data)}:{expected_bytes}')
    sha=hashlib.sha256(data).hexdigest()
    if sha != expected_sha:
        raise SystemExit(f'{label}_SHA256_MISMATCH:{sha}:{expected_sha}')
    if not (data.startswith(b'\xff\xd8\xff') and data.endswith(b'\xff\xd9')):
        raise SystemExit(f'{label}_JPEG_MAGIC_INVALID')
    Path(dst).write_bytes(data)
    print(f'{label}_JPEG_BYTES={len(data)}')
    print(f'{label}_SHA256={sha}')
PY
file /tmp/67-v17-hero.jpg | grep -qi 'JPEG image data'
file /tmp/67-v17-health.jpg | grep -qi 'JPEG image data'

FAILED_STEP=cutover_to_v17_assets
MUTATED=1
cp /tmp/67-v17-hero.jpg "$NEW_HERO"
cp /tmp/67-v17-health.jpg "$NEW_HEALTH"
python3 - <<'PY'
from pathlib import Path
p=Path('/67/src/pages/AdminDashboard.v14.css')
s=p.read_text()
if 'SIX SEVEN ADMIN V17 — CLEAN CINEMATIC ASSET CUTOVER' in s:
    raise SystemExit('V17_ALREADY_PRESENT')
if 'admin-v15-hero.jpg' not in s or 'admin-v15-health.jpg' not in s:
    raise SystemExit('V15_ASSET_REFERENCES_NOT_FOUND')
s=s.replace('admin-v15-hero.jpg','admin-v17-hero.jpg')
s=s.replace('admin-v15-health.jpg','admin-v17-health.jpg')
s += r'''

/* SIX SEVEN ADMIN V17 — CLEAN CINEMATIC ASSET CUTOVER
   Verified complete JPEGs; no salvage, no EOI repair, no truncated payload. */
...
.syshealth__zone-right{
  ...
  background-repeat:no-repeat!important;
  filter:saturate(1.03) contrast(1.08) brightness(.84)!important;
}
/* Keep the V16 reference composition; only remove the visual corruption. */
.ov-header-v16{background:#05080d!important}
CSS
p.write_text(s)
PY
...
grep -q "SIX SEVEN ADMIN V17 — CLEAN CINEMATIC ASSET CUTOVER" "$TARGET_CSS"
...
HERO_BYTES=$(wc -c < "$NEW_HERO" | tr -d ' ')
HEALTH_BYTES=$(wc -c < "$NEW_HEALTH" | tr -d ' ')
...
echo "TRUNCATED_ASSET_SALVAGE=NO"
echo "HERO_CORRUPTION_REMOVED=YES"
echo "HEALTH_CORRUPTION_REMOVED=YES"
echo "NEW_ASSET_FILENAMES=YES"
