...
ASSET_COMMIT=91f80aeafc370175134bdc4316fee690b0d95e14
...
BACKUP_DIR="/tmp/67-admin-v17.1-backup-$$"
FAILED_STEP=init
MUTATED=0
...
rollback(){
  ...
  if [ "$MUTATED" = "1" ]; then
    ...
    npm run build >/tmp/67-admin-v17.1-rollback-build.log 2>&1 || true
  ...
  echo "ERROR=V17_1_DIRECT_BINARY_ASSETS_FAILED_EXIT_${code}"
  exit "$code"
}
trap 'rollback $?' ERR
...
FAILED_STEP=download_exact_binary_assets
HERO_URL="https://raw.githubusercontent.com/mohamedamouseo-a11y/67-patchs/${ASSET_COMMIT}/assets/v17-clean/admin-v17-hero.jpg"
HEALTH_URL="https://raw.githubusercontent.com/mohamedamouseo-a11y/67-patchs/${ASSET_COMMIT}/assets/v17-clean/admin-v17-health.jpg"
curl -fsSL "$HERO_URL" -o /tmp/67-v17.1-hero.jpg
curl -fsSL "$HEALTH_URL" -o /tmp/67-v17.1-health.jpg

FAILED_STEP=verify_exact_binary_assets
python3 - <<'PY'
from pathlib import Path
import hashlib
specs = [
    ('HERO','/tmp/67-v17.1-hero.jpg',11751,'9f0d19b07bb0af0494b5768c7d43a21cb4565f3c05b096c3b665fcecc5d4da65'),
    ('HEALTH','/tmp/67-v17.1-health.jpg',4154,'f2c167f8c9f9b54508dc00aad9f956321627361d70ed53a2a37866ad5b35c9db'),
]
for label,path,expected_bytes,expected_sha in specs:
...
PY
file /tmp/67-v17.1-hero.jpg | grep -qi 'JPEG image data'
file /tmp/67-v17.1-health.jpg | grep -qi 'JPEG image data'

FAILED_STEP=cutover_to_verified_binary_assets
MUTATED=1
cp /tmp/67-v17.1-hero.jpg "$NEW_HERO"
cp /tmp/67-v17.1-health.jpg "$NEW_HEALTH"
python3 - <<'PY'
from pathlib import Path
p=Path('/67/src/pages/AdminDashboard.v14.css')
s=p.read_text()
if 'SIX SEVEN ADMIN V17.1 — DIRECT VERIFIED BINARY ASSETS' in s:
    raise SystemExit('V17_1_ALREADY_PRESENT')
...
/* SIX SEVEN ADMIN V17.1 — DIRECT VERIFIED BINARY ASSETS
   Clean JPEG binaries are stored directly in 67-patchs and SHA256-verified before cutover. */
...
.syshealth__zone-right{
  background-image:linear-gradient(90deg,rgba(5,9,14,.24),transparent 22%,transparent 82%,rgba(5,9,14,.08)),url('../assets/admin-v17-health.jpg')!important;
  background-size:cover!important;
...
[ -n "$HERO_DIST" ]
[ -n "$HEALTH_DIST" ]
python3 - "$HERO_DIST" "$HEALTH_DIST" <<'PY'
from pathlib import Path
import hashlib,sys
specs=[
 ('HERO',sys.argv[1],11751,'9f0d19b07bb0af0494b5768c7d43a21cb4565f3c05b096c3b665fcecc5d4da65'),
 ('HEALTH',sys.argv[2],4154,'f2c167f8c9f9b54508dc00aad9f956321627361d70ed53a2a37866ad5b35c9db')]
for label,path,size,sha in specs:
 d=Path(path).read_bytes()
 if len(d)!=size or hashlib.sha256(d).hexdigest()!=sha:
  raise SystemExit(f'{label}_DIST_ASSET_MISMATCH')
PY
...
grep -q "SIX SEVEN ADMIN V17.1 — DIRECT VERIFIED BINARY ASSETS" "$TARGET_CSS"
