...
ASSET_COMMIT=PLACEHOLDER_ASSET_COMMIT
...
rollback(){
  ...
  echo "ERROR=V17_2_DIRECT_BINARY_ASSETS_FAILED_EXIT_${code}"
  exit "$code"
}
trap 'rollback $?' ERR
...
curl -fsSL "${BASE_URL}/admin-v17-hero.jpg" -o /tmp/67-v17.2-hero.jpg
curl -fsSL "${BASE_URL}/admin-v17-health.jpg" -o /tmp/67-v17.2-health.jpg
...
import hashlib
specs=[
('HERO','/tmp/67-v17.2-hero.jpg',11751,'9f0d19b07bb0af0494b5768c7d43a21cb4565f3c05b096c3b665fcecc5d4da65'),
('HEALTH','/tmp/67-v17.2-health.jpg',4154,'f2c167f8c9f9b54508dc00aad9f956321627361d70ed53a2a37866ad5b35c9db'),
]
for label,path,size,sha in specs:
    data=Path(path).read_bytes()
    if len(data)!=size:
        raise SystemExit(f'{label}_JPEG_SIZE_MISMATCH:{len(data)}:{size}')
    actual=hashlib.sha256(data).hexdigest()
    if actual!=sha:
        raise SystemExit(f'{label}_SHA256_MISMATCH:{actual}:{sha}')
...
s=p.read_text()
if 'SIX SEVEN ADMIN V17.2 — EXACT BINARY CUTOVER' in s:
    raise SystemExit('V17_2_ALREADY_PRESENT')
...
/* SIX SEVEN ADMIN V17.2 — EXACT BINARY CUTOVER
   Byte-exact clean JPEGs, verified by SHA256 before mutation. */
...
grep -q "SIX SEVEN ADMIN V17.2 — EXACT BINARY CUTOVER" "$TARGET_CSS"
