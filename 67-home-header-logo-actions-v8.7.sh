...
BACKUP="/tmp/67-home-header-logo-actions-v8-7-backup-$(date +%Y%m%d-%H%M%S)"
...
if '/assets/logo-67.png' not in s:
    raise SystemExit('ERROR: Header JSX is not wired to /assets/logo-67.png')
if not re.search(r'className=[\'\"]h67-logo[\'\"]', s):
    raise SystemExit('ERROR: .h67-logo wrapper not found')
print('Header logo wiring validated')
PY
...
for marker in ('67_HOME_HEADER_LOGO_V8_5','67_HOME_HEADER_LOGO_V8_6','67_HOME_HEADER_V8_7'):
    s = re.sub(
...
block = r'''
/* 67_HOME_HEADER_V8_7_START */
/* Desktop-only correction:
   1) make the approved full 67 logo unmistakably visible at the far RIGHT;
   2) reorder LEFT controls physically as: Logout | Search | Notification.
   Keep center navigation and hero untouched. */
@media (min-width: 1281px) {
  ...
  /* Any internal wrapper that contains the logo must not become the logo's
     containing block or clip it. This makes .h67-header the positioning reference. */
  .h67-header *:has(.h67-logo) {
    overflow: visible !important;
  }
  ...
  /* Find the real common parent of Search + Bell + Logout using Lucide SVG classes,
     and force physical left-to-right ordering without moving the group itself. */
  .h67-header *:has(> button:has(.lucide-search)):has(> button:has(.lucide-bell)):has(> button:has(.lucide-log-out)) {
