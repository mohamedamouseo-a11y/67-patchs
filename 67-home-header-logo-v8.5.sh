...
BACKUP="/tmp/67-home-header-logo-v8-5-backup-$(date +%Y%m%d-%H%M%S)"
...
if '/assets/logo-67.png' not in s:
    raise SystemExit('ERROR: HomeStoreHeader.jsx is not currently wired to /assets/logo-67.png; stopping without changes')
print('Header JSX already uses /assets/logo-67.png')
PY
...
# Idempotent: remove only this logo-display block if it already exists.
s = re.sub(
    r'\n?/\* 67_HOME_HEADER_LOGO_V8_5_START \*/.*?/\* 67_HOME_HEADER_LOGO_V8_5_END \*/\n?',
    '\n',
    s,
    flags=re.S,
)
...
block = r'''
/* 67_HOME_HEADER_LOGO_V8_5_START */
/* Desktop-only: show the approved full Six Seven logo at the far-right of the
   already-approved header without moving the nav/search/logout layout. */
@media (min-width: 1281px) {
  ...
  /* Target only the direct wrapper that owns the approved logo image. */
  .h67-header *:has(> .h67-logo-image) {
...
echo "HOME_HEADER_LOGO_V8_5_APPLIED"
